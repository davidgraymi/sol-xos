...
describe("sol-xos", () => {
  ...
  describe("successful game, player 1 creates and wins", () => {
    ...
    it("should join an existing game", async () => {
      const p2InitBalance = await provider.connection.getBalance(
        playerTwo.publicKey
      );
      ...
      const p2EndBalance = await provider.connection.getBalance(
        playerTwo.publicKey
      );
      ...
      assert.equal(
        p2EndBalance,
        p2InitBalance - wagerAmount,
        "Player 2 balance should decrease by the exact wager amount"
      );
      assert.equal(
        endPot,
        potTotal,
      ...
      assert.equal(
        p1.toString(),
        playerOne.publicKey.toString(),
        "Player 1 should be the player 2 public key"
      );
      assert.equal(
        p2.toString(),
        playerTwo.publicKey.toString(),
        "Player 2 should be the player 2 public key"
      );
      assert.isTrue(
    ...
    it("should play a move", async () => {
      ...
      assert.equal(
        pot,
        potTotal,
        "Game account balance should be the same as before"
      );
      assert.equal(
        p1.toString(),
        playerOne.publicKey.toString(),
        "Player 1 should be the same as before"
      );
      assert.equal(
        p2.toString(),
        playerTwo.publicKey.toString(),
        "Player 2 should be the same as before"
      );
      assert.isTrue(
        state.playing ? true : false,
        "Game state should be playing"
    ...
    it("player 1 should win the game", async () => {
      ...
      assert.equal(
        p1EndBalance,
        p1OriginBalance + wagerAmount,
        "Player 1 balance should increase by the wager amount"
      );
      assert.equal(
        p2EndBalance,
        p2OriginBalance - wagerAmount,
        "Player 2 balance should decrease by the wager amount"
      );
  ...
  describe("basic successful game, player 1 creates and player 2 wins", () => {
    ...
    it("player 2 should win the game", async () => {
      ...
      assert.equal(
        p1EndBalance,
        p1OriginBalance - wagerAmount,
        "Player 1 balance should decrease by the wager amount"
      );
      assert.equal(
        p2EndBalance,
        p2OriginBalance + wagerAmount,
        "Player 2 balance should increase by the wager amount"
      );
      assert.isNull(gameAccountInfo, "Game account should be closed");
    });
  ...
  describe("basic forfeit, player 1 creates and player 2 forfeits", () => {
    const playerOne: Keypair = Keypair.generate();
    const playerTwo: Keypair = Keypair.generate();
    let uniqueId = new anchor.BN(Date.now());
    let [pda] = PublicKey.findProgramAddressSync(
